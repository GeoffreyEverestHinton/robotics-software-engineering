...
本项目基于TurtleBot机器人平台，整合**SLAM建图、AMCL自主定位导航、YOLO视觉检测、Kinect深度相机感知、五轴机械臂URDF建模**全套功能，实现机器人环境建图、实时定位、自主导航行驶、环境目标视觉检测及机械臂模型仿真配置，适用于室内自主移动机器人开发调试。
...
在TurtleBot原生机器人模型基础上，新增五轴机械臂URDF仿真模型，用于机器人整机仿真与运动规划开发。

### 4\.1 模型文件路径

```Plain Text
/home/turing/catkin_ws/src/turtlebot/turtlebot_description/urdf
```

### 4\.2 新增模型文件操作

进入对应路径，创建并编辑五轴机械臂xacro模型文件：

```Plain Text
cd /home/turing/catkin_ws/src/turtlebot/turtlebot_description/urdf
gedit arm_5joint.urdf.xacro
```

在打开的文件中写入**五轴机械臂URDF建模参数**，完成机械臂模型新增配置，可用于RViz仿真、运动学分析、抓取算法开发。
...
2. 建图时需低速移动机器人，避免速度过快导致地图畸变、重影。

3. 视觉导航需保证RGB\-D图像对齐正常，否则会出现坐标匹配偏差，影响对位精度。

4. 新增URDF模型后，需重新编译工作空间，确保模型被ROS识别加载。

5. 每次打开新终端，建议执行`source ~/catkin_ws/devel/setup.bash`加载环境变量。
